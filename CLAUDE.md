...
- Port 3600 · `spot-price.service` · `spot-price-collector.timer` (every 15 min). Units tracked at `deploy/spot-price.service`, `deploy/spot-price-collector.service`, `deploy/spot-price-collector.timer` (finding #6487) — edit there, then `sudo cp deploy/<unit> /etc/systemd/system/ && sudo systemctl daemon-reload`.
...
- `deploy` — the VPS-wide deploy script at `~/.local/bin/deploy` (shared by every project, not in this repo): merges the worktree branch to master, pushes to forgejo + restarts `spot-price.service` (the collector is timer-driven and picks up new code on its next fire). The forgejo post-receive hook rebuilds the backend and **rsyncs the whole `frontend/` tree** (index.html + styles.css + js/) to `/var/www/html/porssi/` — `frontend/` holds only browser assets, so publishing it wholesale is safe. (Adding a frontend file requires no hook change; adding a non-asset file to `frontend/` would publish it, so keep tests/config in `backend/`.)
- nginx: `location /porssi` serves the static SPA and sets CSP (and re-lists server-level security headers — nginx `add_header` is all-or-nothing); `location /porssi/api/` proxies to `http://127.0.0.1:3600/api/` (prefix stripped; longest-prefix match keeps API paths out of the SPA `try_files` fallback). Snippet with both blocks: `deploy/nginx-porssi.conf`. Live copy is in `/etc/nginx/sites-enabled/default`.
