...
- `.env`: `index.ts` loads the project-root `.env` by path (`utils/project-env.ts` → `../../.env` from `src/` or `dist/`), same as the collector — never `cwd/.env`, since `npm run dev` and the unit run from `backend/`. Loaded only under `isMainModule`, so importing `index.ts` in tests reads no `.env`; systemd `EnvironmentFile` values win (dotenv does not override).
